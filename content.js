(function () {
  ...
  function startCaptureSession(target, event, context) {
    cancelSession();
    const session = {
    ...
    window.setTimeout(() => {
      ...
      emitCellSelected("", "空单元格", session.clickContext);
      finalizeSession(session, "success", "空单元格");
    }, CAPTURE_TIMEOUT_MS);
  }
  ...
  function runCaptureAttempt(session, target, stepIndex) {
    ...
    for (const probe of probes) {
      ...
      if (probe.key === "formula" && ["cell_address_token", "empty", "not_useful"].includes(evaluation.reason)) {
        stepFormulaStale = true;
      }

      if (evaluation.status !== "success") continue;

      emitCellSelected(text, probe.source, session.clickContext);
    ...
    if (stepIndex >= EARLY_EMPTY_MIN_STEP_INDEX && session.emptyLikelyHits >= EARLY_EMPTY_REQUIRED_HITS) {
      emitCellSelected("", "空单元格", session.clickContext);
      finalizeSession(session, "success", "空单元格(快速)");
    }
  }
  ...
  function evaluateCandidate(text, sourceKey) {
    ...
    if (sourceKey === "formula" && isLikelyCellAddressToken(text)) return { status: "reject", reason: "cell_address_token" };
  ...
  function emitCellSelected(text, source, context) {
